...
def render_notifications(screen, font):
    ...
    for notification in active_toast_notifications[:]:
        ...
        if elapsed > notification["duration"] - 500:  # Start fading 500ms before end
            notification["alpha"] = max(0, 255 * (1 - (elapsed - (notification["duration"] - 500)) / 500))
        ...
        text_alpha.fill((255, 255, 255, notification["alpha"]), special_flags=pygame.BLEND_RGBA_MULT)
    ...
    if active_popup:
        ...
        active_popup["close_rect"] = pygame.Rect(close_x, close_y, close_text.get_width(), close_text.get_height())
